    ...
    <div>
        <div class="nx-flex-direction-ld-mgmt nx-flex-flow-wrap-ld-mgmt " style="padding-top:10px">
            <div class="card-width-ld-mgmt" v-for="(lead ,listIndex) in todayLeads.leadsData" :key="listIndex">
              ...
              <div v-if="openLeadData === true" class="card-width-ld-mgmt">
                ...
                self.getJsonpResult = "request failed";
                 this.openLeadData = false
                    ...
                    if(res.data.errCode !== 4) {
                        ...
                        /**
                            openLeadData is true when response have data when no data it is false
                         */
                        if (res.data.errCode === 841) {
                            self.openLeadData = true
                        } else {

                            self.openLeadData = false

                        // let openLeads = self.readDataFromServer[self.readDataFromServer.length - 1];
                    
                            for(let i = 0; i < self.readDataFromServer.length; i++) {
                                if ('leadcount' in self.readDataFromServer[i]) {
                                    break;
                                } else {
