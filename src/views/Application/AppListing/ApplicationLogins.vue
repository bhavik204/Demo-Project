    ...
    <div>
        ...
        <div class="nx-flex-direction nx-flex-flow-wrap tab-wrap-width" style="padding-top:10px;">
            <div class="card-width-app" v-for="(listData, index) in obj" :key="index">
              ...
              <div v-if="draftLogins == false" class="card-width-app">
...
export default {
                ...
                data: [
                   {
                        name:'Rahul Soni',
                        appno:'0012TRA1243',
                        plantype:'Increasing Insurance Plan',
    ...
    created() {
          ...
          console.log(me.$store.state.agentDetails._id)
           ...
           this.status = 'login'
                 this.GET('getAgentproposal/'+ this._id +'?proposalStatus=login', response => {
                      this.obj = (response.data.errMsg[0])
         console.log(this.obj)
                     var errcodeAPI= -1
          if(response.data.errCode == errcodeAPI){
               this.draftLogins = true; 
                        modal.toast({
